...
Computer2::Computer2(Board* board, Colour colour) :
  PlayerController{ board, colour } { }

// Level 2: Prioritizes checks and captures
TurnResult Computer2::doTurn() {
  std::string command;
  ...
  //   else if (command == "move") {
      ...
      for (auto& m: myLegalMoves) {
        int moveValue = getBoard()->evaluateMove(m, getColour(), 0);
        if (moveValue > 0) {
